...
$(function() {
	$('#pagination').bootstrapPaginator({
		...
		pageUrl: function(type, page, current) {
			return '/checkBuyPlanInfo.do?pageNum=' + page;
		},
			...
			<tbody>
				<c:forEach items="${requestScope.pageInfo.list }" var="map"
					...
					<tr>
						...
						<td>${num.count }</td>
						<td>${map.planer_name }</td>
						<td>${map.plan_date }</td>
						<td>${map.buyer_name }</td>
						<td>${map.buy_date }</td>
						<td>${map.num }</td>
						<td>${map.supplier_name }</td>
						<td> <a  id="detailBtn" href="<%=basePath%>/getDetailPlanInfo.do?plan_id=${map.pid}"
							class="tablelink"> 详情</a>
							<a href="<%=basePath%>/jumpModifySupplier.do?plan_id=${map.pid}" class="tablelink">修改供应商</a>
							<a href="<%=basePath%>/setOrderCheck.do?plan_id=${map.pid}" class="tablelink">提交</a></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
