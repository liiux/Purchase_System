...
$(function() {
	$('#pagination').bootstrapPaginator({
		...
		pageUrl: function(type, page, current) {
			return '/receiveInfo.do?pageNum=' + page;
		},
			...
			<tbody>
				<c:forEach items="${requestScope.pageInfo.list }" var="map"
					...
					<tr>
						...
						<td>${map.linkman }</td>
						<td>${map.phone }</td>
						<td> <a   href="<%=basePath%>/receiveDetailInfo.do?plan_id=${map.pid}"
							class="tablelink"> 详情</a>
							 <a   href="<%=basePath%>/setPlanReceive.do?plan_id=${map.pid}"
							class="tablelink"> 提交</a></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
