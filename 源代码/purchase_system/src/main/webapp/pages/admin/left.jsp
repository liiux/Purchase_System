	...
	<dl class="leftmenu">
		...
		<c:forEach items="${sessionScope.oneMenuList }" var="OneMenu">
			<dd>
				<!-- 一级菜单 -->
				<div class="title">
					<span><img src="<%=basePath%>/images/leftico03.png" /></span>${OneMenu.oneName }
				</div>
				<!-- 二级菜单 -->
				<ul class="menuson">
					<c:forEach items="${pageScope.OneMenu.twoMenuList }" var="TwoMenu">
						<li>
							<div class="header">
								<cite></cite> <a href="${TwoMenu.twoUrl }?user_id=${sessionScope.user_id}" target="rightFrame">${TwoMenu.twoName }</a>
								<i></i>
							</div>
						</li>
					</c:forEach>
